...
$result=mysql_query("SELECT id,titulo,age,fecha,path,ext,hora,pathv FROM videos ORDER BY fecha DESC,hora DESC ;"); 
    $i=0;
    $mm="";
	...
	$mm = str_replace("/foto","/afpv",$row['path']);
        
  
        echo "<td class=tabval><img src=img/blank.gif width=1 height=2></td>";
	echo "<td class=tabval><b>".$row['titulo']."</b></td>";
        echo "<td class=tabval><a class=n target=text onclick=\"h(this);\" href=text.php?action=vid&id=".$row['id']."><img src=$mm width=60 height=40 > <br>";
	echo "<small>".$row['fecha']."-".$row['hora']." </small></td>";
