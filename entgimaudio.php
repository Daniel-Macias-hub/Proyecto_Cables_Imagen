...
<meta http-equiv="Refresh" content="60,audios.php">
</head>
<body>
...
//$result=mysql_query("SELECT id,st,cat,tit,aut,fec,key,path FROM audios WHERE aut=\"ADELA\" ORDER BY fec DESC;");
//$result=mysql_query("SELECT id,st,cat,tit,aut,fec,audios.key,path FROM audios WHERE audios.key =\"90.5\" ORDER BY fec DESC;");
$result=mysql_query("SELECT id,st,cat,tit,aut,fec,audios.key,path FROM audios WHERE cat=\"ENTGIMAUDIO\"  ORDER BY fec DESC;");
$i=0;
while( $row=mysql_fetch_array($result) )
	...
	echo "</tr>";
        ...
        echo "<td class=tabval><b>".$row['tit']." - ".$row['cat']."</b></td>";
        echo "<td class=tabval><img src=img/blank.gif width=5 height=10></td>";
        echo "<td class=tabval><small>".$row['fec']."</small></td>";
	echo "<td class=tabval><a class=n target=text onclick=\"h(this);\" href=".$row['path']."><span class = red >[mp3]</span></td>";
