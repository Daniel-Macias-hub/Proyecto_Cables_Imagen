...
<meta http-equiv="Refresh" content="90,urgentes.php">
</head>
<body>
...
$result=mysql_query("SELECT titulo,age,cat,fecha,hora,path,urge FROM cables WHERE (urge=\"3\" OR urge=\"2\"  OR urge=\"1\" ) ORDER BY fecha DESC,hora DESC LIMIT 250;");    
$i=0;
if(!session_id()) session_start();
$server = $_SESSION['server'];
	...
	echo "<tr class=tr_mmp valign=bottom>";
        if($row['urge']=="1") echo "<td bgcolor=#cc0000 background='img/strichel.gif' colspan=7><img src=img/blank.gif width=2 height=5></td>";
        if($row['urge']=="2") echo "<td bgcolor=#cc0000 background='img/strichel.gif' colspan=7><img src=img/blank.gif width=2 height=5></td>";     

	if($row['urge']=="3") echo "<td bgcolor=#ffff66 background='img/strichel.gif' colspan=7><img src=img/blank.gif width=2 height=5></td>";
        if($row['urge']=="4") echo "<td bgcolor=#f87820 background='img/strichel.gif' colspan=7><img src=img/blank.gif width=2 height=5></td>";
