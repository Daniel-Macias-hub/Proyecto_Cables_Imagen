...
$result=mysql_query("SELECT id,age,cat,tit,aut,fec,path,capt2 FROM fotos WHERE age=\"NOTIMEX\" ORDER BY fec DESC;"); 
    $i=0;
    $mm="";
	...
	$mm = str_replace("/foto","/tumb",$row['path']);
        $strtit = $row['tit'];
        $fecorta = $row['fec'];
        $fecorta = substr($fecorta,5,11);
        if (strlen($strtit) > 200)  $strtit = substr($strtit, 0, 200) . '...';
