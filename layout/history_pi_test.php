...
$station = 10000000001; // Estación Estados (Pruebas)
$today = date('Y-m-d');
...
// Solo ejecutar la consulta si el usuario presionó APLICAR
if ($busqueda_activa) {
...
<head>
    ...
    <title>Historial PI - Pruebas (Estados)</title>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .page-header i { font-size: 1.6rem; color: var(--corp-red); }
        .badge-station {
            margin-left: auto;
            font-size: 0.7rem;
            font-weight: 700;
        ...
        .details__content { padding: 0; }
        .details[open] .details__content { padding: 24px; }
...
<div class="page-header">
    ...
    <h1>Historial de Pautas — Estados</h1>
    <span class="badge-station"><i class="fas fa-broadcast-tower"></i> Estación: 10000000001</span>
</div>

<?php if ($error_db): ?>
    <div style="background: rgba(227,6,19,0.1); color: #E30613; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
        <i class="fas fa-exclamation-triangle"></i> <?php echo htmlspecialchars($error_db); ?>
    </div>
<?php endif; ?>
...
<div class="filters-bar">
    <form method="get">
        <div class="filter-group">
            ...
            <input type="date" name="fecha" value="<?php echo htmlspecialchars($fecha); ?>" class="filter-input">
        </div>
        <div class="filter-group">
            <label>Organizar por</label>
            <select name="orden" class="filter-input">
                <option value="programa_asc" <?php echo $orden == 'programa_asc' ? 'selected' : ''; ?>>Alfabético (A - Z)</option>
                <option value="programa_desc" <?php echo $orden == 'programa_desc' ? 'selected' : ''; ?>>Alfabético (Z - A)</option>
                <option value="hora" <?php echo $orden == 'hora' ? 'selected' : ''; ?>>Hora Programada</option>
            </select>
        </div>
        <div class="filter-group">
...
<!-- Banner inicial: no hay búsqueda activa -->
<div style="text-align:center; padding:80px 30px; opacity:0.7;">
    ...
    <h2 style="font-size:1.3rem; font-weight:800; margin-bottom:10px;">Seleccione los filtros para consultar</h2>
    <p style="font-size:0.95rem; color:var(--text-color); opacity:0.7;">Configure la Fecha, Programa y Orden, y presione <strong>"APLICAR"</strong> para mostrar resultados.</p>
</div>
<?php else: ?>
...
<div class="results-label">
    Resultados para: <span><?php echo htmlspecialchars($fecha_display); ?></span>
</div>
...
<div class="empty-hist">
    ...
    <h2>Sin registros</h2>
    <p>No hay transmisiones para la fecha seleccionada en la Estación Estados.</p>
</div>
<?php else: ?>
            ...
            <div class="details__content">
                <div class="record-grid">
                    <?php foreach($records as $item):
                        ...
                        $status = strtolower(isset($item['status']) ? $item['status'] : '');
                        $isError = (strpos($status, 'no ') !== false || strpos($status, 'cancel') !== false);
                    ?>
                        <div class="record-card">
                            <div class="record-col">
                                <span class="col-label">Orden</span>
                                <span class="col-value val-id">#<?php echo htmlspecialchars($item['id'] ? $item['id'] : $item['trorden']); ?></span>
                            </div>
                            <div class="record-col">
                                <span class="col-label">Producto</span>
                                <span class="col-value val-prod"><?php echo htmlspecialchars($item['idTR']); ?></span>
                            </div>
                            <div class="record-col col-client">
                                <span class="col-label">Cliente</span>
                                <span class="col-value"><?php echo htmlspecialchars($item['client'] ? $item['client'] : '---'); ?></span>
                            </div>
                            <div class="record-col">
                                <span class="col-label">Hora Real</span>
                                <span class="col-value"><i class="far fa-clock" style="opacity:0.5; margin-right:4px;"></i><?php echo htmlspecialchars($item['hrt'] ? substr($item['hrt'], 0, 5) : '--:--'); ?></span>
                            </div>
                            <div class="record-col">
                                <span class="col-label">Estatus</span>
                                <span class="col-value val-status <?php echo $isError ? 'status-err' : ''; ?>">
                                    ...
                                    <?php echo htmlspecialchars(isset($item['status']) ? $item['status'] : ''); ?>
                                </span>
                            </div>
                            <div class="record-col col-obs">
                                <span class="col-label">Observaciones</span>
                                <span class="col-value" style="font-size:0.95rem; opacity:0.8;"><?php echo htmlspecialchars($item['obs'] ? $item['obs'] : 'Sin observaciones'); ?></span>
                            </div>
                            <div class="actions-col">
                                <a href="text2.php?action=pi&id=<?php echo $item['id']; ?>&nom=<?php echo urlencode($item['idTR']); ?>" target="mainIframe" class="btn-action text <?php echo !$hasText ? 'disabled' : ''; ?>" title="Ver Nota">
                                ...
                                <a href="player.php?audio=<?php echo urlencode($item['rutamp3']); ?>&prog=<?php echo urlencode($prog_name); ?>" target="mainIframe" class="btn-action audio <?php echo !$hasAudio ? 'disabled' : ''; ?>" title="Escuchar Audio">
